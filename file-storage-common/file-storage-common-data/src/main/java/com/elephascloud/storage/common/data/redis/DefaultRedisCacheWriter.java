package com.elephascloud.storage.common.data.redis;
...
/**
 * {@link RedisCacheWriter} implementation capable of reading/writing binary data from/to Redis in
 * {@literal standalone} and {@literal cluster} environments. Works upon a given {@link
 * RedisConnectionFactory} to obtain the actual {@link RedisConnection}. <br /> {@link
 * DefaultRedisCacheWriter} can be used in {@link RedisCacheWriter#lockingRedisCacheWriter(RedisConnectionFactory)
 * locking} or {@link RedisCacheWriter#nonLockingRedisCacheWriter(RedisConnectionFactory)
 * non-locking} mode. While {@literal non-locking} aims for maximum performance it may result in
 * overlapping, non atomic, command execution for operations spanning multiple Redis interactions
 * like {@code putIfAbsent}. The {@literal locking} counterpart prevents command overlap by setting
 * an explicit lock key and checking against presence of this key which leads to additional requests
